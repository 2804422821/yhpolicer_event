"""
优抚一件事-人员管理-因公牺牲人员序列化器 - 1对1扩展，扁平化展示
"""
...
class PersonalSacrificeSerializer(CustomModelSerializer):
    """
    因公牺牲人员 扁平化序列化器（合并基本信息字段 + 牺牲特有字段）
    """
    ...
    pension_type = serializers.CharField(source='info.pension_type', max_length=5, required=False, allow_blank=True, default="0", label="抚恤类别")
    ...
    # PersonalSacrifice 自身字段
    sacrifice_time = serializers.DateField(required=False, allow_null=True, label="牺牲时间")
    sacrifice_reason = serializers.CharField(required=False, allow_blank=True, allow_null=True, label="牺牲原因")
    confirmed_sacrifice_time = serializers.DateField(required=False, allow_null=True, label="认定因公牺牲时间")
    martyr_approval_time = serializers.DateField(required=False, allow_null=True, label="批烈时间")
    is_martyr = serializers.CharField(max_length=1, required=False, allow_blank=True, default="0", label="是否烈士")
    approval_document_number = serializers.CharField(max_length=200, required=False, allow_blank=True, allow_null=True, label="认定文号")
    ...
    class Meta:
        model = PersonalSacrifice
        fields = [
            "id", "info_id",
            "policer_number", "name", "gender", "id_card", "birth_date",
            "nation", "pension_type", "identity_type", "job_level", "dept_name",
            "work_date", "political_status", "brief_achievements",
            "sacrifice_time", "sacrifice_reason", "confirmed_sacrifice_time",
            "martyr_approval_time", "is_martyr", "approval_document_number",
...
class PersonalSacrificeCreateSerializer(PersonalSacrificeSerializer):
    pass


class PersonalSacrificeUpdateSerializer(PersonalSacrificeSerializer):
    class Meta(PersonalSacrificeSerializer.Meta):
        read_only_fields = PersonalSacrificeSerializer.Meta.read_only_fields + ["info_id"]


class PersonalSacrificeResource(resources.ModelResource):
    """
    因公牺牲人员 导入/导出 Resource
    """
    ...
    sacrifice_time = Field(attribute="sacrifice_time", column_name="牺牲时间")
    sacrifice_reason = Field(attribute="sacrifice_reason", column_name="牺牲原因")
    confirmed_sacrifice_time = Field(attribute="confirmed_sacrifice_time", column_name="认定因公牺牲时间")
    martyr_approval_time = Field(attribute="martyr_approval_time", column_name="批烈时间")
    is_martyr = Field(attribute="is_martyr", column_name="是否烈士")
    approval_document_number = Field(attribute="approval_document_number", column_name="认定文号")
    ...
    class Meta:
        ...
        fields = (
            ...
            "work_date", "brief_achievements", "sacrifice_time", "sacrifice_reason",
            "confirmed_sacrifice_time", "martyr_approval_time", "is_martyr",
            "approval_document_number", "enjoy_hero_model_benefits",
        )
        export_order = fields
        import_id_fields = []
        skip_unchanged = True
    ...
    def before_import_row(self, row, **kwargs):
        ...
        if id_card:
            ...
            if not PersonalSacrifice.objects.filter(info=info).exists():
                PersonalSacrifice.objects.create(info=info)
        return row
